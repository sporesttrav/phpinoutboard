...
function allowStatus(uid,sid) {
	...
	//alert(postData);
	microAjax('aj/admin_saveCalAccess.php', function(pageData){$div('error').innerHTML= pageData},postData)
}
</script>
...
?>
    <tr id="row">
      ...
      <?
		foreach($status_arr as $status_row) {
			$status_query = sprintf("select * from emp_calendar_access 
  ...
  <h4>A checked box allows access from the status list on the &quot;Schedule Event&quot; page</h4>
</div>
<div id="error">&nbsp;</div>
